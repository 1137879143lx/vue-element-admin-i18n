...
router.post('/', async (req, res) => {
  try {
    ...
    if (!CustomerId || !ShortName || !FullName) {
      // 检查客户信息是否完整
      return res.status(400).json({ code: 400, msg: 'CustomerId、ShortName 和 FullName 不能为空' }) // 返回错误信息
    }
    const customer = await Customer.findOne({ CustomerId }) // 查找客户是否已存在
    if (customer) {
      return res.status(400).json({ code: 400, msg: '该客户已存在' }) // 返回错误信息
    }
    await Customer.create({ CustomerId, ShortName, FullName, Address, Contact, Tel, Email, Phone, Tax, CurrencyCode }) // 创建客户
    res.json({ code: 200, msg: '客户创建成功' }) // 返回成功信息
  } catch (err) {
    res.status(500).json({ code: 500, msg: err.message }) // 返回错误信息
  }
})
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!ShortName || !FullName) {
      // 检查客户信息是否完整
      return res.status(400).json({ code: 400, msg: 'ShortName 和 FullName 不能为空' }) // 返回错误信息
    }
    const customer = await Customer.findById(req.params.id) // 查找指定 ID 的客户
    if (!customer) {
      return res.status(404).json({ code: 404, msg: '客户不存在' }) // 返回错误信息
